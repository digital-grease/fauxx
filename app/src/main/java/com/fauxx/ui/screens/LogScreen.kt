    ...
    Column(
        ...
        Row(
            ...
            IconButton(onClick = { showExportMenu = true }) {
                Icon(Icons.Default.Download, stringResource(R.string.log_export_cd))
                DropdownMenu(
        ...
        Column(modifier = Modifier.padding(12.dp)) {
            ...
            AnimatedVisibility(
                ...
                Column {
                    ...
                    Spacer(Modifier.height(8.dp))
                    DetailRow(
                        stringResource(R.string.log_detail_type),
                        stringResource(entry.actionType.displayNameRes())
                    )
                    DetailRow(
                        stringResource(R.string.log_detail_category),
                        stringResource(entry.category.displayNameRes())
                    )
                    DetailRow(
                        stringResource(R.string.log_detail_detail),
                        entry.detail.removePrefix("[${entry.category}] ")
                    )
                    DetailRow(
                        stringResource(R.string.log_detail_time),
                        FULL_DATE_FORMAT.format(Date(entry.timestamp))
                    )
                    DetailRow(
                        stringResource(R.string.log_detail_status),
                        if (entry.success) {
                            stringResource(R.string.common_success)
                        } else {
                            stringResource(R.string.common_failed)
                        }
                    )
                }
            }
